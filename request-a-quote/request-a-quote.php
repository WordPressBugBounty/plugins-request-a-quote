...
if (!class_exists('Request_a_quote')):
	...
	final class Request_a_quote {
		...
		public function __clone() {
			_doing_it_wrong(__FUNCTION__, __('Cheatin&#8217; huh?', 'request-a-quote') , '1.0');
		}
		/**
		...
		private function define_constants() {
			define('REQUEST_A_QUOTE_VERSION', '2.5.0');
		...
		public function load_plugin_textdomain() {
			...
			$localmo = REQUEST_A_QUOTE_PLUGIN_DIR . '/lang/' . $mofile;
			$globalmo = WP_LANG_DIR . '/request-a-quote/' . $mofile;
			if (file_exists($globalmo)) {
				load_textdomain('request-a-quote', $globalmo);
			} elseif (file_exists($localmo)) {
				load_textdomain('request-a-quote', $localmo);
			} else {
				load_plugin_textdomain('request-a-quote', false, REQUEST_A_QUOTE_PLUGIN_DIR . '/lang/');
			}
		}
		/**
		...
		public function display_settings() {
			...
			add_menu_page(__('Quotes Settings', 'request-a-quote') , __('Quotes Settings', 'request-a-quote') , $settings_pages_cap, $this->app_name, array(
				$this,
				'display_getting_started_page'
			));
			add_submenu_page($this->app_name, __('Getting Started', 'request-a-quote') , __('Getting Started', 'request-a-quote') , $settings_pages_cap, $this->app_name);
			add_submenu_page($this->app_name, __('Glossary', 'request-a-quote') , __('Glossary', 'request-a-quote') , $settings_pages_cap, $this->app_name . '_glossary', array(
				$this,
				'display_glossary_page'
			));
			add_submenu_page($this->app_name, __('Settings', 'request-a-quote') , __('Settings', 'request-a-quote') , $settings_pages_cap, $this->app_name . '_settings', array(
				$this,
				'display_settings_page'
			));
			add_submenu_page($this->app_name, __('Shortcodes', 'request-a-quote') , __('Shortcodes', 'request-a-quote') , $settings_pages_cap, $this->app_name . '_shortcodes', array(
				$this,
				'display_shortcodes_page'
			));
			add_submenu_page($this->app_name, __('Forms', 'request-a-quote') , __('Forms', 'request-a-quote') , $settings_pages_cap, $this->app_name . '_forms', array(
				$this,
				'display_forms_page'
			));
			add_submenu_page($this->app_name, __('Custom Fields', 'request-a-quote') , __('Custom Fields', 'request-a-quote') , $settings_pages_cap, $this->app_name . '_cust_fields', array(
				$this,
			...
			if (!empty($emd_lic_settings)) {
				...
				if ($show_lic_page == 1 && function_exists('emd_show_license_page')) {
					add_submenu_page($this->app_name, __('Licenses', 'request-a-quote') , __('Licenses', 'request-a-quote') , 'manage_options', $this->app_name . '_licenses', array(
						$this,
		...
		public function display_support_page() {
			emd_display_support($this->app_name, 2, 'request-a-quote');
		}
		...
		public function display_cust_fields_page() {
			emd_lite_get_operations('cust_fields', __('Custom Fields', 'request-a-quote') , 'request_a_quote');
		}
		/**
