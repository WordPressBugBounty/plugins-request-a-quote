...
class Emd_Notifications {
	...
	public function __construct($app) {
		...
		$this->notify_field_list = Array(
			'active' => array(
				'id' => 'active',
				'name' => __('Active', 'request-a-quote') ,
				'info' => '',
				'type' => 'checkbox',
				'isset' => 1
			) ,
			'ev_front_add' => array(
				'id' => 'ev_front_add',
				'name' => __('Frontend Add', 'request-a-quote') ,
				'info' => '',
				'type' => 'checkbox',
				'group' => 'event',
				'isset' => 1
			...
			'ev_back_add' => array(
				'id' => 'ev_back_add',
				'name' => __('Backend Add', 'request-a-quote') ,
				'info' => '',
				'type' => 'checkbox',
				'group' => 'event',
				'isset' => 1
			...
			'ev_change' => array(
				'id' => 'ev_change',
				'name' => __('Change', 'request-a-quote') ,
				'info' => '',
				'type' => 'checkbox',
				'group' => 'event',
				'isset' => 1
			...
			'ev_delete' => array(
				'id' => 'ev_delete',
				'name' => __('Delete', 'request-a-quote') ,
				'info' => '',
				'type' => 'checkbox',
				'group' => 'event',
				'isset' => 1
			...
			'ev_change_val' => array(
				'id' => 'ev_change_val',
				'name' => __('Change Value', 'request-a-quote') ,
				'info' => __('If change value is empty, this notification is triggered in every value change.', 'request-a-quote') ,
				'type' => 'text',
				'isset' => 1
			) ,
			'user_msg' => array(
				'id' => 'user_msg',
				'name' => __('User Message', 'request-a-quote') ,
				'info' => '',
				'type' => 'header',
				'isset' => 0
			) ,
			'user_send_to' => array(
				'id' => 'user_send_to',
				'name' => __('Send To', 'request-a-quote') ,
				'info' => '',
				'type' => 'multicheck',
				'mtype' => 'user',
				'isset' => 0
			...
			'user_from_name' => array(
				'id' => 'user_from_name',
				'name' => __('From Name', 'request-a-quote') ,
				'info' => '',
				'type' => 'text',
				'mtype' => 'user',
				'isset' => 0
			...
			'user_from_email' => array(
				'id' => 'user_from_email',
				'name' => __('From Email', 'request-a-quote') ,
				'info' => '',
				'type' => 'text',
				'mtype' => 'user',
				'isset' => 0
			...
			'user_reply_to' => array(
				'id' => 'user_reply_to',
				'name' => __('Reply To', 'request-a-quote') ,
				'info' => '',
				'type' => 'text',
				'mtype' => 'user',
				'isset' => 0
			...
			'user_subject' => array(
				'id' => 'user_subject',
				'name' => __('Subject', 'request-a-quote') ,
				'info' => '',
				'type' => 'text',
				'mtype' => 'user',
				'isset' => 0
			...
			'user_message' => array(
				'id' => 'user_message',
				'name' => __('Message', 'request-a-quote') ,
				'info' => '',
				'type' => 'editor',
				'mtype' => 'user',
				'isset' => 0
			...
			'admin_msg' => array(
				'id' => 'admin_msg',
				'name' => __('Admin Message', 'request-a-quote') ,
				'info' => '',
				'type' => 'header',
				'isset' => 0
			) ,
	...
	public function notify_static_callback($args) {
				...
				if(!empty($ent_list)){
					foreach ($ent_list as $kent => $vent) {
						...
						if($klist == 'attrs' && $vent['display_type'] == 'user'){
							$$klist.= sprintf(__('%s Login Username','request-a-quote'),esc_html($this->ent_list[$ment]['label'])) . ": <b>{" . esc_html($ment) . "_login_username}</b> ";
							$$klist.= sprintf(__('%s Login Password Reset Link','request-a-quote'),esc_html($this->ent_list[$ment]['label'])) . ": <b>{" . esc_html($ment) . "_login_password_reset_link}</b> ";
							$$klist.= sprintf(__('%s Verification Link','request-a-quote'),esc_html($this->ent_list[$ment]['label'])) . ": <b>{" . esc_html($ment) . "_verify_link}</b> ";
						}
					}
				}
				$badd = 1;
		...
		$globs = (empty($globs) ? __("Not available", 'request-a-quote') : rtrim($globs, ","));
		$attrs = (empty($attrs) ? __("Not available", 'request-a-quote') : rtrim($attrs, ","));
		$taxs = (empty($taxs) ? __("Not available", 'request-a-quote') : rtrim($taxs, ","));
		$rels = (empty($rels) ? __("Not available", 'request-a-quote') : rtrim($rels, ","));
		$coms = (empty($coms) ? __("Not available", 'request-a-quote') : rtrim($coms, ","));
		$fields = "<table><tr><th colspan=2>";
		$fields.= sprintf(__('Use template tags below to customize your email. Taxonomy and relationship tags produce link(s) to the related record(s). For no link tag, add %s to relationship or taxonomy tag. For example, for {mytag%s} tag produces a no link version of the tag. Check glossary tab for definitions.', 'request-a-quote') , '_nl', '_nl');
		$fields.= "</th></tr>";
		if ($this->notify_init_list[$args['notify_id']]['level'] == 'rel') {
			foreach ($ents as $ment) {
				$fields .= "<tr><th>" . esc_html($this->ent_list[$ment]['label']) . ' ' . __('Builtin', 'request-a-quote') . "</th>";
				$builtin = __('Title', 'request-a-quote') . ": <b>{" . esc_html($ment) . "_title}</b> " . __('Permalink', 'request-a-quote') . ": <b>{" . esc_html($ment) . "_permalink}</b> " . __('Edit Link', 'request-a-quote') . ": <b>{" . esc_html($ment) . "_edit_link}</b> " . __('Delete Link', 'request-a-quote') . ": <b>{delete_link}</b> " . __('Excerpt', 'request-a-quote') . ": <b>{" . esc_html($ment) . "_excerpt}</b> " . __('Content', 'request-a-quote') . ": <b>{" . esc_html($ment) . "_content}</b> " . __('Author Display Name', 'request-a-quote') . ": <b>{" . esc_html($ment) . "_author_dispname}</b> " . __('Author NickName', 'request-a-quote') . ": <b>{" . esc_html($ment) . "_author_nickname}</b> " . __('Author First Name','request-a-quote') . ": <b>{" . esc_html($ment) . "_author_fname}</b> " . __('Author Last Name','request-a-quote') . ": <b>{" . esc_html($ment) . "_author_lname}</b> " . __('Author Username','request-a-quote') . ": <b>{" . esc_html($ment) . "_author_login}</b> " . __('Author Bio','request-a-quote') . ": <b>{" . esc_html($ment) . "_author_bio}</b> " . __('Author Googleplus','request-a-quote') . ": <b>{" . esc_html($ment) . "_author_googleplus}</b> " . __('Author Twitter','request-a-quote') . ": <b>{" . esc_html($ment) . "_author_twitter}</b> ";
				$fields .= "<td>" . $builtin . "</td></tr>";
			}
		}
		else {
			$builtin = __('Title', 'request-a-quote') . ": <b>{title}</b> " . __('Permalink', 'request-a-quote') . ": <b>{permalink}</b> " . __('Edit Link', 'request-a-quote') . ": <b>{edit_link}</b> " . __('Delete Link', 'request-a-quote') . ": <b>{delete_link}</b> " . __('Excerpt', 'request-a-quote') . ": <b>{excerpt}</b> " . __('Content', 'request-a-quote') . ": <b>{content}</b> " . __('Author Display Name', 'request-a-quote') . ": <b>{author_dispname}</b> " . __('Author NickName', 'request-a-quote') . ": <b>{author_nickname}</b> " . __('Author First Name','request-a-quote') . ": <b>{author_fname}</b> " . __('Author Last Name','request-a-quote') . ": <b>{author_lname}</b> " . __('Author Username','request-a-quote') . ": <b>{author_login}</b> " . __('Author Bio','request-a-quote') . ": <b>{author_bio}</b> " . __('Author Googleplus','request-a-quote') . ": <b>{author_googleplus}</b> " . __('Author Twitter','request-a-quote') . ": <b>{author_twitter}</b> ";
			$fields .= "<tr><th>" . __('Builtin', 'request-a-quote') . "</th>";
			$fields .= "<td>" . $builtin . "</td></tr>";
		}

		$site_params = __('Site Title','request-a-quote') . " <b>{site_name}</b> " . __('Tagline','request-a-quote') . " <b>{site_tagline}</b> " . __('Site Address','request-a-quote') . " <b>{site_link}</b> ";
		$site_params = apply_filters('emd_notify_site_params',$site_params,$this->app,$ents);	
		$fields .= "<tr><th>" . __('Site', 'request-a-quote') . "</th><td>" . $site_params . "</td></tr>";
		

		$fields .= "<tr><th>" . __('Globals', 'request-a-quote') . "</th>
			<td>" . $globs . "</td>
			</tr>
			<tr><th>" . __('Attributes', 'request-a-quote') . "</th>
			<td>" . $attrs . "</td>
			</tr>
			<tr><th>" . __('Taxonomies', 'request-a-quote') . "</th>
			<td>" . $taxs . "</td>
			</tr>
			<tr><th>" . __('Relationships', 'request-a-quote') . "</th>
			<td>" . $rels . "</td>
			</tr>
			<tr><th>" . __('Comments', 'request-a-quote') . "</th>
			<td>" . $coms . "</td>
			</tr>
			</table>";
		$allowed_html = array(
	...
	public static function display_settings_notify($app, $notify_init_list) {
		if (!empty($notify_init_list)) {
			...
			echo '</h2>';
			echo '<p>' . esc_html__('Below is your notification list', 'request-a-quote') . ':</p>';
