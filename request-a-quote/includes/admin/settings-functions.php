...
if (!function_exists('emd_show_settings_page')) {
	function emd_show_settings_page($app){
		...
		<a href="#" class="add-new-h2 emd-import" style="padding:6px 10px;"><?php echo esc_html('Import', 'request-a-quote');?></a>
		<a href="<?php echo esc_url($export_url); ?>" class="add-new-h2" style="padding:6px 10px;"><?php echo esc_html('Export', 'request-a-quote');?></a>
		<?php
		}
		else {
		?>
		<a href="#" class="add-new-h2 upgrade-pro" style="padding:6px 10px;"><?php echo esc_html('Import', 'request-a-quote');?></a>
		<a href="#" class="add-new-h2 upgrade-pro" style="padding:6px 10px;"><?php echo esc_html('Export', 'request-a-quote');?></a>
		<?php
		} ?>
		</h2>
		<?php	
		$tabs['entity'] = esc_html__('Entities', 'request-a-quote');
		$new_tax_list = Array();
		...
		if(!empty($new_tax_list)){	
			...
			$tabs['taxonomy'] = esc_html__('Taxonomies', 'request-a-quote');
		}
		$tabs = apply_filters('emd_add_settings_tab',$tabs,$app);
		$tabs['tools'] = esc_html__('Tools', 'request-a-quote');
		$active_tab = isset($_GET['tab']) ? sanitize_text_field($_GET['tab']) : 'entity';
		if(isset($_GET['settings-updated']) && $_GET['settings-updated'] == true){
			echo '<div id="message" class="updated">' . esc_html__('Settings Saved.','request-a-quote') . '</div>';
		}
		echo '<style>
	...
	function emd_ent_map_tab($app,$active_tab,$shc_list){
		...
		foreach($map_ents as $kent => $myent){
			...
			if($is_ent_public !== false){
				...
				$temp_options = Array('right' => __('Right Sidebar','request-a-quote'),'left' => __('Left Sidebar','request-a-quote'), 'full' => __('Full Width','request-a-quote'));
				...
				echo "</select><p class='description'>" . sprintf(esc_html__('Sets the template for single %s.','request-a-quote'),esc_html(strtolower($myent['label']))) . "</p></td></tr>";
				echo "<tr><th scope='row'><label for='ent_map_list_" . esc_attr($kent) . "_single_container'>";
				echo esc_html__('Container type','request-a-quote');
				echo '</label></th><td>';
				...
				$container_options = Array('container' => __('Fixed','request-a-quote'),'container-fluid' => __('Full','request-a-quote'));
				...
				echo ">";
				echo "<p class='description'>" . sprintf(esc_html__('Hides the previous and next %s links on the frontend for single %s.','request-a-quote'),esc_html(strtolower($myent['label'])),esc_html(strtolower($myent['label']))) . "</p></td></tr>";
				echo "<tr><th scope='row'><label for='ent_map_list_" . esc_attr($kent) . "_hide_edit_link'>";
				echo esc_html__('Hide edit links','request-a-quote');
				echo '</label></th><td>';
				...
				echo ">";
				echo "<p class='description'>" . sprintf(esc_html__('Hides edit %s link on the frontend for single %s.','request-a-quote'),esc_html(strtolower($myent['label'])),esc_html(strtolower($myent['label']))) . "</p></td></tr>";
			}
			if($ent_list[$kent]['archive_view']){	
				echo "<tr><th scope='row'><label for='ent_map_list_" . esc_attr($kent) . "_archive_temp'>";
				echo esc_html__('Archive template','request-a-quote');
				echo '</label></th><td>';
				...
				echo "</select><p class='description'>" . sprintf(esc_html__('Sets the template for archive %s.','request-a-quote'),esc_html(strtolower($myent['label']))) . "</p></td></tr>";
				echo "<tr><th scope='row'><label for='ent_map_list_" . esc_attr($kent) . "_archive_container'>";
				echo esc_html__('Archive container type','request-a-quote');
				echo '</label></th><td>';
				...
				echo "</select><p class='description'>" . sprintf(esc_html__('Change this if the sidebars are getting out of your page boundry. Fixed type provides a responsive fixed width container for archive %s. Full type provides a full width container, spanning the entire width of the viewport for archive %s.','request-a-quote'),esc_html(strtolower($myent['label'])),esc_html(strtolower($myent['label']))) . "</p></td></tr>";
				echo "<tr><th scope='row'><label for='ent_map_list_" . esc_attr($kent) . "_hide_archive_page_nav'>";
				echo esc_html__('Hide page navigation','request-a-quote');
				echo '</label></th><td>';
				...
				echo ">";
				echo "<p class='description'>" . sprintf(esc_html__('Hides the page navigation links on the frontend for archive %s.','request-a-quote'),esc_html(strtolower($myent['label']))) . "</p></td></tr>";
			}
			if($shc_list['remove_vis'] != 1){
				//Show all attributes and ability to disable them
				$fields_table = '';
				$fields_options_pub = Array('show' => __('Enable','request-a-quote'),
						'hide' => __('Disable','request-a-quote'),
						'hide_frontend' => __('Show only in Admin','request-a-quote')
				);
				$fields_options_not_pub = Array('show' => __('Enable','request-a-quote'),
						'hide' => __('Disable','request-a-quote'),
				);
				$fields_options_no_disable = Array('show' => __('Enable','request-a-quote'),
						'hide_frontend' => __('Show only in Admin','request-a-quote')
				);
				if($is_ent_public !== false){
				...
				if(!empty($shc_list['frontedit'])){
					...
					$visitor_access = Array('show' => __('Show','request-a-quote'),'not_show'=>__('Do not show','request-a-quote'));
					$role_access = Array('edit'=>__('Allow Edit','request-a-quote'),'show' => __('Show','request-a-quote'),'not_show'=>__('Do not show','request-a-quote'));
				}
				...
				if(!empty($ent_list[$kent]['featured_img'])){
					$fields_table .= "<tr><td style='style='font-weight:500;'>" . __('Featured Image','request-a-quote') . "</td>";
				...
				if(!empty($rels)){
					foreach($rels as $myrel){
						...
						$rel_options = Array('show' => __('Enable','request-a-quote'),
									'hide' => __('Disable','request-a-quote'),
									'hide_frontend' => __('Show only in Admin','request-a-quote')
								);
				...
				if(!empty($fields_table)){
					...
					echo esc_html__('Attributes','request-a-quote');
					echo '</label></th><td>';
					echo "<p class='description'>" . esc_html__('Enable: Display this attribute on everywhere</br>Disable: Remove this attribute from everywhere</br>Show only in Admin: This attribute is still enabled on admin area. If you want to hide this attribute on the frontend forms go to Forms tab.','request-a-quote') . "</p></br>";
					...
					echo ">";
					echo "<tr><th style='text-align:center;'>" . esc_html__('Attribute','request-a-quote') . "</th><th style='text-align:center;'>" . esc_html__('Visibility','request-a-quote'). "</th>";
					if(!empty($shc_list['frontedit'])){
						echo "<th style='text-align:center;'>" . esc_html__('Visitor','request-a-quote') . "</th>";
			...
			if (!empty($has_limitby_auth_type[$kent])){
				...
				if($is_ent_limitby === true && $is_ent_public !== false){
					...
					echo esc_html__('Remove access restrictions on frontend','request-a-quote');
					echo '</label></th><td>';
					...
					echo ">";
					echo "<p class='description'>" . sprintf(esc_html__('Shows all %s to logged in and non-logged in users regardless of content access permissions assigned to their user role.','request-a-quote'),esc_html(strtolower($myent['label']))) . "</p></td></tr>";
				}	
			}
			if(!empty($file_attrs[$kent])){
				foreach($file_attrs[$kent] as $fattr => $fval){
					...
					echo '</h3>';
					echo "<span class='description' style='padding-left:10px;color:#777;'>- " . esc_html__('Use the options below to customize file uploads.','request-a-quote') . "</span></th></tr>";
					if(empty($fval['max_file_uploads']) || (!empty($fval['max_file_uploads']) &&  $fval['max_file_uploads'] != 1)){
						...
						echo esc_html__('Maximum files','request-a-quote');
						echo '</label></th><td>';
						$max_files = "";
						...
						echo "<p class='description'>" . esc_html__('Sets the maximum number of allowable file uploads.','request-a-quote') . "</p></td></tr>";
					}
					echo "<tr><th scope='row'><label for='ent_map_list_" . esc_attr($kent) . "_max_file_size_" . esc_attr($fattr) . "'>";
					echo esc_html__('Maximum file size','request-a-quote');
					echo '</label></th><td>';
					$max_file_size = "";
					...
					echo "<p class='description'>" . sprintf(esc_html__('Sets the maximum uploadable file size in kilobytes. Your server allows up to %s KB. Leave it blank to use server limit.','request-a-quote'),esc_html($server_size)) . "</p></td></tr>";
					echo "<tr><th scope='row'><label for='ent_map_list_" . esc_attr($kent) . "_file_exts_" . esc_attr($fattr) . "'>";
					echo esc_html__('Allowed file extensions','request-a-quote');
					echo '</label></th><td>';
					$file_exts = "";
					...
					echo "<p class='description'>" . esc_html__('Sets the file extensions allowed to upload. Seperate each extension by a comma. For example: jpg,png,pdf','request-a-quote') . "</p></td></tr>";
				}
			}
			if(!empty($country_attrs[$kent])){
				foreach($country_attrs[$kent] as $cattr => $cval){
					...
					echo '</h3>';
					echo "<span class='description' style='padding-left:10px;color:#777;'>- " . esc_html__('Use the option below to select a default.','request-a-quote') . "</span></th></tr>";
					echo "<tr><th scope='row'><label for='ent_map_list_" . esc_attr($kent) . "_default_country_" . esc_attr($cattr) . "'>";
					echo esc_html__('Default Country','request-a-quote');
					echo '</label></th><td>';
			...
			if(!empty($state_attrs[$kent])){
				foreach($state_attrs[$kent] as $sattr => $sval){
					...
					echo esc_html__('Default State','request-a-quote');
					echo '</label></th><td>';
			...
			if(!empty($comment_list) && !empty($comment_list[$kent])){
				...
				echo '</h3>';
				echo "<span class='description' style='padding-left:10px;color:#777;'>- " . sprintf(esc_html__('Use the options below to customize %s %s.','request-a-quote'),esc_html(strtolower($myent['label'])),esc_html(strtolower($comment_list[$kent]['label']))) . "</span></th></tr>";
				echo "<tr><th scope='row'><label for='ent_map_list_" . esc_attr($kent) . "_comment_placeholder'>";
				echo esc_html__('Set placeholder','request-a-quote');
				echo "</label></th><td>";
				...
				$comm_placeholder = __('Join the discussion...','request-a-quote');
				...
				echo esc_html__('Set the depth of threaded (nested) comments','request-a-quote');	
				echo '</label></th><td>';
				...
				echo ">" . esc_html__('All','request-a-quote') . "</option>";	
				echo '<option value=1';
				if($thread_depth == 1){
					echo " selected";
				}
				echo ">" . esc_html__('None','request-a-quote') . "</option>";	
				for($i=2;$i<=10;$i++){
				...
				echo esc_html__('Posts per page','request-a-quote');
				echo '</label></th><td>';
				$per_page_val = '10';
				...
				if(!empty($cust_roles)){
					...
					echo esc_html__('Show labels for user roles','request-a-quote');
					echo '</label></th><td>';
					...
					echo "><p class='description'>" . sprintf(esc_html__('Displays the default plugin user roles (%s) next to the user name in a response box when a user belonging to that role leaves a reply.','request-a-quote'),esc_html($roles)) . "</p></td></tr>";
				}
				echo "<tr><th scope='row'><label for='ent_map_list_" . esc_attr($kent) . "_comments_attach'>";
				echo esc_html__('Enable file uploads','request-a-quote');
				echo '</label></th><td>';
				...
				echo esc_html__('Enable file uploads for non-logged in users','request-a-quote');
				echo '</label></th><td>';
...
if (!function_exists('emd_show_map_attrs')) {
	function emd_show_map_attrs($app,$myent,$ent_map_variables,$ent_map_list){
		foreach($myent['attrs'] as $mattr){
			...
			echo "<tr><th scope='row'></th><td><table><th scope='row'><label>" . esc_html__('Frontend map settings','request-a-quote') . "</th><td></td></tr>
				<th scope='row'><label for='ent_map_list_" . esc_attr($mattr_key) . "_width'>" . esc_html__('Width','request-a-quote') . "</th><td><input id='" . esc_attr($app) . "_ent_map_list_" . esc_attr($mattr_key) . "_width' name='" . esc_attr($app) . "_ent_map_list[" . esc_attr($mattr_key) . "][width]' type='text' value='" . esc_attr($width) . "'></input><p class='description'>" . esc_html__('Sets the map width.You can use \'%\' or \'px\'. Default is 100%.','request-a-quote') . "</p></td></tr>";
			echo "<tr><th scope='row'><label for='ent_map_list_" . esc_attr($mattr_key) . "_height'>" . esc_html__('Height','request-a-quote') . "</th><td><input id='" . esc_attr($app) . "_ent_map_list_" . esc_attr($mattr_key) . "_height' name='" . esc_attr($app) . "_ent_map_list[" . esc_attr($mattr_key) . "][height]' type='text' value='" . esc_attr($height) ."'></input><p class='description'>" . esc_html__('Sets the map height. You can use \'px\'. Default is 480px.','request-a-quote') . "</p></td></tr>";
			echo "<tr><th scope='row'><label for='ent_map_list_" . esc_attr($mattr_key) . "_zoom'>" . esc_html__('Zoom','request-a-quote') . "</th><td><select id='" . esc_attr($app) . "_ent_map_list_" . esc_attr($mattr_key) . "_zoom' name='" . esc_attr($app) . "_ent_map_list[" . esc_attr($mattr_key) . "][zoom]'>";
			for($i=20;$i >=1;$i--){
				echo "<option value='" . esc_attr($i) . "'";
				if($zoom == $i){
					echo " selected";
			...
			echo "</select></td></tr>";
			echo "<tr><th scope='row'><label for='ent_map_list_" . esc_attr($mattr_key) . "_map_type'>" . esc_html__('Type','request-a-quote') . "</th><td><select id='" . esc_attr($app) . "_ent_map_list_" . esc_attr($mattr_key) . "_map_type' name='" . esc_attr($app) . "_ent_map_list[" . esc_attr($mattr_key) . "][map_type]'>";
			...
			echo "</select></td></tr>";
			echo "<tr><th scope='row'><label for='ent_map_list_" . esc_attr($mattr_key) . "_marker'>" . esc_html__('Marker','request-a-quote') . "</th><td><input id='" . esc_attr($app) . "_ent_map_list_" . esc_attr($mattr_key) . "_marker' name='" . esc_attr($app) . "_ent_map_list[" . esc_attr($mattr_key) . "][marker]' type='checkbox' value=1 $marker></input></td></tr>";
			echo "<tr><th scope='row'><label for='ent_map_list_" . esc_attr($mattr_key) . "_load_info'>" . esc_html__('Display info window on page load','request-a-quote') . "</th><td><input id='" . esc_attr($app) . "_ent_map_list_" . esc_attr($mattr_key) . "_load_info' name='" . esc_attr($app) . "_ent_map_list[" . esc_attr($mattr_key) . "][load_info]' type='checkbox' value=1 $load_info></input></td></tr>";
		...
		<?php	echo '<form method="post" action="options.php">';
			...
			foreach($tax_list_vals as $ktax => $mytax){
				...
				if($shc_list['remove_vis'] != 1){
					...
					echo esc_html__('Availability','request-a-quote');
					echo '</label></th><td>';
					if(empty($shc_list['frontedit'])){
						...
						$tax_options = Array('show' => __('Enable','request-a-quote'),
								'hide' => __('Disable','request-a-quote'),
								'hide_frontend' => __('Show only in Admin','request-a-quote')
								);
						...
						echo "</select>";
						echo "<p class='description'>" . esc_html__('Enable: Display this taxonomy on everywhere</br>Disable: Remove this taxonomy from everywhere</br>Show only in Admin: This taxonomy is still enabled on admin area. If you want to hide this taxonomy on the frontend forms go to Forms tab.','request-a-quote') . "</p></br>";
					} else {
						echo "<table class='widefat striped'";
						...
						echo ">";
						echo "<tr><th style='text-align:center;'>" . esc_html__('Visibility','request-a-quote'). "</th>";
						if(!empty($shc_list['frontedit'])){
							echo "<th style='text-align:center;'>" . esc_html__('Visitor','request-a-quote') . "</th>";
							foreach($app_custom_roles as $crole){
								echo "<th style='text-align:center;'>" . esc_html($crole) . "</th>";
				...
				if(!empty($mytax['archive_view'])){	
					...
					echo "</select><p class='description'>" . esc_html__('Sets the template for the posts which belong to this taxonomy.','request-a-quote') . "</p></td></tr>";
					...
					echo ">";
					echo "<p class='description'>" . esc_html__('Hides the page navigation links on the frontend for archive posts.','request-a-quote') . "</p></td></tr>";
				}
				echo '</tbody></table>';
				echo '</div></div></li>';
			}
...
if (!function_exists('emd_tools_tab')) {
	function emd_tools_tab($app,$active_tab,$shc_list){
		if ( ! empty( $_GET['action'] ) && ! empty( $_REQUEST['_wpnonce'] ) && wp_verify_nonce( sanitize_text_field($_REQUEST['_wpnonce']), 'install_pages_action' ) ) {
			...
			echo '<div class="updated inline"><p>' . esc_html__( 'All missing pages were installed successfully.', 'request-a-quote' ) . '</p></div>';
		}
		...
		echo esc_html__('Install pages','request-a-quote');
		echo '</label></th><td>';
		echo '<a href="' .  wp_nonce_url( admin_url('admin.php?page=' . esc_attr($app) . '_settings&tab=tools&action=install_pages'), 'install_pages_action' ) . '" class="button install_pages">' . esc_html__( 'Install pages', 'request-a-quote' ) . '</a>';
		echo "<p class='description'>" . sprintf(esc_html__('This tool will install all the missing %s pages. Pages already defined and set up will not be replaced.','request-a-quote'),esc_html($shc_list['app'])) . "</p></td></tr>";
		echo "<tr><th scope='row'><label for='tools_disable_emd_templates'>";
		echo esc_html__('Disable EMD Templating System','request-a-quote');
		echo '</label></th><td>';
		...
		echo ">";
		echo "<p class='description'>" . esc_html__('This tool will delete all settings/options data including setup assistant created plugin pages when using the "Delete" link on the plugins screen.','request-a-quote') . "</p></td></tr>";
		echo "<tr><th scope='row'><label for='tools_remove_data'>";
		echo esc_html__('Delete all data','request-a-quote');
		echo '</label></th><td>';
		...
		echo "<p class='description'>" . sprintf(esc_html__('This tool will delete all %s data and related taxonomies including setup assistant created plugin pages when using the "Delete" link on the plugins screen. As a best practice we recommend you to backup your database before selecting this option.','request-a-quote'),esc_html($ent_labels)) . "</p></td></tr>";
		echo "<tr><th scope='row'><label for='tools_custom_css'>";
                echo esc_html__('Custom CSS','request-a-quote');
                echo "</label></th><td>";
                ...
                echo "<p class='description'>" . esc_html__('Custom CSS allows you to add your own styles or override the default CSS of this plugin. The CSS code written here is only applied to this plugin\'s frontend pages.','request-a-quote') . "</p></td></tr>";
		echo "<tr><th scope='row'><label for='tools_custom_js'>";
                echo esc_html__('Custom JS','request-a-quote');
                echo "</label></th><td>";
                ...
                echo "<p class='description'>" . esc_html__('Custom JS allows you to add your own JavaScript. The JavaScript code written here is only applied to this plugin\'s frontend pages.','request-a-quote') . "</p></td></tr>";
		echo '</tbody></table>';
		submit_button(); 
		echo '</form></div>';
