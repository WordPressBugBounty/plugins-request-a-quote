...
if (!function_exists('emd_form_builder_lite_submit_form')) {
	function emd_form_builder_lite_submit_form($app, $fcontent) {
		...
		if(!empty($_POST)){
			foreach ($_POST as $postkey => $mypost) {
				if (isset($mypost) && $mypost != '' && $postkey != 'form_data') {
					if(is_array($mypost)){
						...
						if (is_array($uploads)) {
							foreach ($uploads as $myfileupload) {
								if (isset($myfileupload['path']) && $myfileupload['error'] == 0) {
									...
									if(preg_match('/csv/',$pmtype)){
										//fix csv injection
										$formula_chars = array( "=", "+", "-", "@", "|", "%" );
										if (($fp = fopen($myfileupload['path'], "r")) !== FALSE) {
											$dt = array();
											$changed = 0;
											while(($data = fgetcsv($fp)) !== FALSE) {
												foreach($data as $k => $line){
													if(in_array(substr($line,0,1), $formula_chars)){
														$changed = 1;
														$data[$k] = ltrim($line, '=+-@|%');
													}
												}
	...
	elseif($fields['shc'] == 'adv_table'){
		if($type == 'header'){
			...
			if($fields['adv_table']['adv_show_export']){
				...
				<li data-type="">
				<a href="#"><i class="fa fa-fw fa-check"></i>' . __('Current Page', 'request-a-quote') . '</a>
				</li>
				<li data-type="all">
				<a href="#"><i class="fa fa-fw"></i>' . __('All', 'request-a-quote') . '</a>
				</li>
				<li data-type="selected">
				<a href="#"><i class="fa fa-fw"></i>' . __('Selected', 'request-a-quote') . '</a>
				</li>
				</ul>';
			}
			$ret .= '</div>
