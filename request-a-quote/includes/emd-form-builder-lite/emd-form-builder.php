...
function emd_show_forms_lite_page($app){
	...
	$create_url = '#'; 
	echo '<a href="' . esc_url($create_url) . '" class="button btn-primary button-primary upgrade-pro">' . esc_html('Create New', 'request-a-quote') . '</a>';
	echo '<a href="#" class="add-new-h2 upgrade-pro" style="padding:6px 10px;">' . esc_html('Import', 'request-a-quote') . '</a>';
	echo '<a href="#" class="add-new-h2 upgrade-pro" style="padding:6px 10px;">' . esc_html('Export', 'request-a-quote') . '</a>';
	echo '</h2>';
	echo '<p>' . esc_html__('Emd Form Builder makes it easy to create simple or advanced forms with a few clicks.','request-a-quote') . ' <a href="https://emdplugins.com/best-form-builder-for-wordpress/?pk_campaign=' . esc_attr($app) . '&pk_kwd=emdformbuilderpagelink" target="_blank">' . esc_html__('Click here to learn more.','request-a-quote') . '</a></p>';
...
function emd_form_builder_lite_layout($app,$form_id,$from='admin'){
	...
	echo '<div class="updated is-dismissible notice emd-form-save-success" style="display:none;"><p><strong>' . esc_html__('Saved successfully.','request-a-quote') . '</strong></p></div>';
	...
	echo '<div class="emd-form-builder-top"><div class="emd-form-builder-center">' . esc_html__('Editing Form:','request-a-quote') . ' ' . esc_html($myform->post_title) . '</div>
		<div class="emd-form-builder-right"><a href="#" id="emd-form-save">' . esc_html__('Save','request-a-quote') . '</a>
	...
	echo '<div class="emd-form-builder-sidebar">
		...
		<button class="emd-form-builder-add-row" data-app="' . esc_attr($app) . '"' . ' data-entity="' . esc_attr($fentity) . '" title="' . esc_html__('Click here to add row','request-a-quote') . '">' . esc_html__('Add Row','request-a-quote') . '</button>
		</div>
		<div class="emd-form-builder-html-fields">
		<button class="emdform-hr-button" data-app="' . esc_attr($app) . '"' . ' data-entity="' . esc_attr($fentity) . '" title="' . esc_html__('Click here to add','request-a-quote') . '">' . esc_html__('Divider','request-a-quote') . '</button>
		<button class="emdform-html-button upgrade-pro" data-app="' . esc_attr($app) . '"' . ' data-entity="' . esc_attr($fentity) . '" data-field="html' . esc_attr($htmlcount) . '" title="' . esc_html__('Click here to add','request-a-quote') . '">' . esc_html__('HTML','request-a-quote') . '</button>
		</div>
		<div class="emd-form-builder-add-page-wrap">';
	if($fcontent['type'] == 'submit'){	
		echo '<button class="emd-form-builder-add-page upgrade-pro" data-app="' . esc_attr($app) . '"' . ' data-entity="' . esc_attr($fentity) . '" title="' . esc_html__('Click here to add a form wizard page','request-a-quote') . '">' . esc_html__('Add Page','request-a-quote') . '</button>';
	}
	echo '<div class="emd-form-builder-pages">
		<div class="emd-form-builder-page active" id="emd-form-builder-page-1" title="' . esc_html__('Click to go to this page','request-a-quote') . '">' . 
		esc_html__('Page 1','request-a-quote') . '</div>';
	if($pcount > 1){
		for($i=2;$i<=$pcount;$i++){
			echo '<div class="emd-form-builder-page" id="emd-form-builder-page-' . esc_attr($i) . '" title="' . esc_html__('Click to go to this page','request-a-quote') . '">' .
				'<a href="#" class="emd-form-builder-page-delete" title="' . esc_html__('Delete','request-a-quote') . '"><span class="field-icons times-circle" aria-hidden="true"></span></a>' .
				sprintf(esc_html__('Page %s','request-a-quote'),esc_html($i)) . '</div>';
		}
	}	
	echo '</div>
		</div>';
	echo '<div class="emd-form-builder-tabs">
		<a href="#" class="emd-form-builder-tab active" id="fields">' . esc_html__('Fields','request-a-quote') . '</a>
		<a href="#" class="emd-form-builder-tab" id="settings">' . esc_html__('Settings','request-a-quote') . '</a>
		</div>
...
function emd_form_builder_lite_fields($app,$fentity,$fcontent){
	...
	$has_user = 0;
	$titl = __('Drag this field to a row.','request-a-quote');
	if(!empty($attr_list[$fentity])){
		...
		if(!empty($blts)){
			foreach($blts as $kblt => $vblt){
				...
				elseif(!in_array($kblt,$glob_list)){
					echo ' upgrade-pro';
					$titl = __('Drag this field to a row - available in premium edition.','request-a-quote');
				}
		...
		if(!empty($glbs)){
			foreach($glbs as $kglb => $vglb){
				...
				echo ' emd-attr" id="' . esc_attr($kglb) . '-btn" data-field="' . esc_attr($kglb) . '" title="' . esc_html__('Drag this field to a row','request-a-quote') . '">' . esc_html($vglb['label']) . '</button>';
			}
		}
		echo '</div>';
		echo '</div>';
	...
	if(!empty($has_user)){
		...
		echo ' emd-attr" id="login_box-btn" data-field="login_box_username" title="' . esc_attr($titl) . '">' . esc_html__('Login Box','request-a-quote') . '</button>';
		echo '</div>';
		echo '</div>';
	}	
}	
function emd_form_builder_lite_get_form_layout($app,$fentity,$ftitle,$fcontent){
	...
	if(!empty($fcontent['layout'])){
		foreach($fcontent['layout'] as $kpage => $cpage){
			if(!empty($cpage['rows'])){
				...
				foreach($cpage['rows'] as $krow => $crow){
					...
					echo '<a href="#" class="emd-form-row-delete" title="' . esc_html__('Delete','request-a-quote') . '"><span class="field-icons times-circle" aria-hidden="true"></span></a>';
					echo '<span class="emd-form-row-info">' . esc_html__('Drag to reorder','request-a-quote') . '</span>';
					...
					foreach($crow as $fcount => $field){
						foreach($field as $kfield => $cfield){
							if(!empty($cfield['show']) && !in_array($kfield, Array('login_box_password','login_box_reg_password','login_box_reg_confirm_password','login_box_reg_username'))){ 
								...
								echo '<a href="#" class="emd-form-field-delete" title="' . esc_html__('Delete','request-a-quote') . '"><span class="field-icons times-circle" aria-hidden="true"></span></a>';
								echo '<span class="emd-form-field-info" style="cursor:pointer;">' . esc_html__('Drag to reorder / Click for settings','request-a-quote') . '</span>';
		...
		echo '<div class="emd-form-row init">
			<a href="#" class="emd-form-row-delete" title="' . esc_html__('Delete','request-a-quote') . '"><span class="field-icons times-circle" aria-hidden="true"></span></a>
			<span class="emd-form-row-info">' . esc_html__('Drag to reorder','request-a-quote') . '</span>
			...
			<div class="emd-form-insert-row">' . esc_html__('Drag fields here','request-a-quote') . 
			'</div>
			</div>
			</div>';
...
function emd_form_builder_lite_layout_html($kfield,$cfield){
	echo '<div class="emd-form-group">
		...
		<input type="text" name="' . esc_attr($kfield) . '" id="' . esc_attr($kfield) . '" class="text emd-input-md emd-form-control html-code" placeholder="' . esc_html__('HTML','request-a-quote') . '" disabled/>
		</div>';
}
	...
	switch($cfield['display_type']){
		...
		case 'hidden':
			...
			elseif(empty($cfield['uniqueAttr'])){
				$attr_lay .= '<div>' . __('Hidden field','request-a-quote') . '</div>';
			}
			break;
		case 'global':
			$attr_lay .= '<div>' . __('Global field','request-a-quote') . '</div>';
			break;
		case 'text':
		default:
			$attr_lay .= '<input type="text" name="' . esc_attr($kfield) . '" id="' . esc_attr($kfield) . '" class="text emd-input-md emd-form-control" placeholder="' . esc_attr($cfield['placeholder']) . '" disabled/>';
...
function emd_form_builder_lite_get_form_field_settings($app,$fentity,$fcontent){
	...
	foreach($all_fields as $kfield => $vfield){
		...
		if(!empty($vfield['display_type']) && $vfield['display_type'] == 'hidden' && $fcontent['type'] == 'submit'){
			...
			echo '<div>' . esc_html__('There are no settings options for hidden fields.','request-a-quote') . '</div>';
		}
		elseif(!empty($vfield['display_type']) && $vfield['display_type'] == 'global' && $fcontent['type'] == 'submit'){
			echo '<input type="hidden" name="fields[' . esc_attr($kfield) . '][label]" class="emd-form-builder-field-label" id="emd-fbl-' . esc_attr($kfield) . '" value="' . esc_html($vfield['label']) . '"/>';
			echo '<div>' . esc_html__('There are no settings options for global fields.','request-a-quote') . '</div>';
		}
		else {
			echo '<div class="emd-form-builder-field-setting label">';
			echo '<label for="' . esc_attr($kfield) . '-label">' . esc_html__('Label','request-a-quote') . '</label>';
			...
			echo '>'; 
			echo '<label class="inline" for="' . esc_attr($kfield) . '-req">' . esc_html__('Required','request-a-quote') . '</label>';
			echo '</div>';
			echo '<div class="emd-form-builder-field-setting size">';
			echo '<label for="' . esc_attr($kfield) . '-size">' . esc_html__('Size','request-a-quote') . '</label>';
			...
			echo '</select>';
			echo '<p class="desc">' . esc_html__('Size column refers to the form elements length relative to the other elements in the same row. Total element size in each row can not exceed 12 units.','request-a-quote') . '</p>';
			echo '</div>';
			echo '<div class="emd-form-builder-field-setting desc">';
			echo '<label for="' . esc_attr($kfield) . '-desc">' . esc_html__('Description','request-a-quote') . '</label>';
			...
			echo '<label for="' . esc_attr($kfield) . '-placeholder">' . esc_html__('Placeholder','request-a-quote') . '</label>';
			...
			echo '<label for="' . esc_attr($kfield) . '-css-class">' . esc_html__('Css Class','request-a-quote') . '</label>';
	...
	foreach($html_fields as $hfield => $hvfield){
		echo '<div class="emd-form-builder-field-settings-wrap emd-field-' . esc_attr($hfield) . '" style="display:none;">
			...
			<label for="emd-field-html">' . esc_html__('HTML','request-a-quote') . '</label>
	...
	foreach($page_fields as $pfield => $pvfield){
		echo '<div class="emd-form-builder-field-settings-wrap emd-field-page' . esc_attr($pfield) . '" style="display:none;">
			...
			<label for="emd-field-step-title">' . esc_html__('Step Title','request-a-quote') . '</label>
		...
		echo '>
			...
			<label for="emd-field-step-desc">' . esc_html__('Step Description','request-a-quote') . '</label>
	...
	if(!empty($has_user)){
		echo '<div class="emd-form-builder-field-settings-wrap emd-field-login_box_username" style="display:none;">
			...
			<label for="login_box-login-label">' . esc_html__('Login Label','request-a-quote') . '</label>
		...
		if(!empty($login_fields['login_box_username']['login_label'])){
			...
			echo esc_html__('Already have an account? Login.','request-a-quote');
		}
		echo  '"></div>
			<div class="emd-form-builder-field-setting label">
			<label for="login_box-register-label">' . esc_html__('Register Label','request-a-quote') . '</label>
		...
		if(!empty($login_fields['login_box_username']['reg_label'])){
			...
			echo esc_html__('Need to create an account? Register.','request-a-quote');
		}
		echo  '"></div>
		<div class="emd-form-builder-field-setting label">
		<label for="login_box-username-label">' . esc_html__('Login Username Label','request-a-quote') . '</label>
		...
		if(!empty($login_fields['login_box_username']['label'])){
			...
			echo esc_html__('Username','request-a-quote');
		}
		echo  '"></div>
		<div class="emd-form-builder-field-setting label">
		<label for="login_box-password-label">' . esc_html__('Login Password Label','request-a-quote') . '</label>
		...
		if(!empty($login_fields['login_box_password']['label'])){
			...
			echo esc_html__('Password','request-a-quote');
		}
		echo  '"></div>
		<div class="emd-form-builder-field-setting label">
		<label for="login_box-redirect-link">' . esc_html__('Redirect Link','request-a-quote') . '</label>
		...
		echo  '">';
		echo '<p class="desc">' . esc_html__('If left empty after login user will be redirected to single entity page.','request-a-quote') . '</p>
		</div>
		...
		echo '>'; 
		echo '<label class="inline" for="login_box-enable_registration">' . esc_html__('Enable Registration','request-a-quote') . '</label>
		</div>
		<div class="emd-form-builder-field-setting label">
		<label for="login_box-reg-username-label">' . esc_html__('Registration Username Label','request-a-quote') . '</label>
		...
		echo  '"></div>
		<div class="emd-form-builder-field-setting label">
		<label for="login_box-reg-password-label">' . esc_html__('Registration Password Label','request-a-quote') . '</label>
		...
		echo  '"></div>
		<div class="emd-form-builder-field-setting label">
		<label for="login_box-reg-confirm-password-label">' . esc_html__('Registration Confirm Password Label','request-a-quote') . '</label>
		...
		if(!empty($login_fields['login_box_reg_confirm_password']['label'])){
			...
			echo esc_html__('Confirm Password','request-a-quote');
		}
		echo  '"></div>
		</div>';
	}
...
function emd_form_builder_lite_get_page(){
	...
	if(!empty($_GET['page_id']) && !empty($_GET['app']) && !empty($_GET['entity'])){
		...
		$page = '<div class="emd-form-builder-page" id="emd-form-builder-page-' . $npid . '" title="' . esc_html__('Click to go to this page','request-a-quote') . '">
			<a href="#" class="emd-form-builder-page-delete" title="' . esc_html__('Delete','request-a-quote') . '"><span class="field-icons times-circle" aria-hidden="true"></span></a>' .
			esc_html__('Page','request-a-quote') . ' ' . esc_attr($npid) . '</div>';
...
function emd_form_builder_lite_get_html(){
	...
	if(!empty($_GET['htmlcount']) && !empty($_GET['app']) && !empty($_GET['entity'])){
		...
		$playout = '<div class="emd-form-row init html">
			...
			<a href="#" class="emd-form-field-delete" title="' . esc_html__('Delete','request-a-quote') . '"><span class="field-icons times-circle" aria-hidden="true"></span></a>
			<span class="emd-form-field-info" style="cursor:pointer;">' . esc_html__('Drag to reorder / Click for settings','request-a-quote') . '</span>
			<div class="emd-form-group">
			<input type="hidden" name="layout[]" value="html' . esc_attr($htmlcount) . '">
			<div id="html' . esc_attr($htmlcount) . '" class="emd-form-html-div"><span class="emd-html-click">' . esc_html__('Click to go to settings','request-a-quote') . '</span>
			<input type="text" class="text emd-input-md emd-form-control html-code" style="display:none;" placeholder="' . esc_html__('HTML','request-a-quote') . '" disabled/>
			</div>
			</div>
			</div>
			</div>
...
function emd_form_builder_lite_login_box($kfield,$cfield){
	if($kfield == 'login_box_username'){
		$lay = '<div class"login_register">
			<input type="text" name="' . esc_attr($kfield) . '" id="' . esc_attr($kfield) . '" class="text emd-input-md emd-form-control" placeholder="' . esc_html__('Login / Register Box','request-a-quote') . '" disabled/>';
		$lay .= '</div></div>';
	}
	else {
		$lay = '';
