			...
			foreach ($matches[1] as $match_tag) {
				if(in_array($match_tag,Array('site_name','site_tagline','site_link'))){
					...
					foreach($mycomments as $mycomm){
						$new .= "<li><div>" . __("Author:","request-a-quote") . $mycomm->comment_author . "&nbsp;&nbsp;";
						$new .= get_comment_date(sprintf(__('%s \a\t %s', 'request-a-quote'),get_option('date_format'),get_option('time_format')),$mycomm->comment_ID) . "</div>";
					...
					if ($myrel['from'] == $mypost->post_type) {
						$conns = $wpdb->get_results($wpdb->prepare("SELECT p2p_to as pid FROM {$wpdb->p2p} WHERE p2p_type=%s AND p2p_from = %s",$new_match_tag,$pid),ARRAY_A);
					}
					else {
						$conns = $wpdb->get_results($wpdb->prepare("SELECT p2p_from as pid FROM {$wpdb->p2p} WHERE p2p_type=%s AND p2p_to = %s",$new_match_tag,$pid),ARRAY_A);
					}
					if(!empty($conns)){
	...
	function emd_get_attr_val($app, $pid, $ptype, $attr_id,$where='') {
		...
		if(!empty($attr_list[$ptype][$attr_id])){
			...
			if ($dtype == 'file') {
				...
				$val = date_i18n(get_option('date_format') . ' ' . get_option('time_format'),strtotime(emd_mb_meta($attr_id, array(), $pid)));
...
if (!function_exists('emd_check_userEmail')) {
	...
	function emd_check_userEmail() {
		...
		$myapp = isset($_GET['myapp']) ? sanitize_text_field($_GET['myapp']) : '';
...
if(!function_exists('emd_get_attachment_layout')){
	function emd_get_attachment_layout($info){
		...
		if($type == 'image'){
			...
			echo "<div class='att-file' style='padding:5px;float:left;'>" . "<a class='att-link thickbox' rel='page' href='" . esc_url($info['url']) . "' title='" . esc_attr($info['title']) . "'><img class='img-thumbnail' style='width:75px;height:75px;' alt='" . esc_attr($img_alt) . "' src='" . esc_url($att_src[0]) . "'/></a>" . "</div>";
		}
		else {
			...
			echo "<div class='att-file' style='padding:5px;float:left;'>" . "<a class='att-link img-thumbnail' style='padding:0;text-decoration:none;' href='" . esc_url($info['url']) . "' target='_blank' title='" . esc_attr($info['title']) . "'><span style='vertical-align:middle;font-size:75px;width:75px;height:75px;' class='dashicons " . esc_attr($dashicon) . "'></span></a>" . "</div>";
		}
	}
}
