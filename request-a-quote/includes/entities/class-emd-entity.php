...
class Emd_Entity {
	...
	public function include_tabs_acc() {
		...
		if ($this->maybe_show_tabs()) {
			...
			$inc_file = constant(strtoupper($this->app) . "_PLUGIN_DIR") . '/includes/entities/' . $fname . '-tabs.php';
	...
	public function change_title($post_id, $post) {
		if ($post->post_type == $this->post_type) {
			if (in_array($post->post_title, Array(
				...
				$class_delimiter = apply_filters('emd_get_title_delimiter', " ", $this->app, $this->post_type);
	...
	public function updated_messages($messages) {
		...
		$messages[$this->post_type] = array(
			...
			1 => sprintf(__('%s updated. <a href="%s">View %s</a>', 'request-a-quote') , $this->sing_label, esc_url(get_permalink($post_ID)) , $this->sing_label) ,
			2 => __('Custom field updated.', 'request-a-quote') ,
			3 => __('Custom field deleted.', 'request-a-quote') ,
			4 => sprintf(__('%s updated.', 'request-a-quote') , $this->sing_label) ,
			/* translators: %s: date and time of the revision */
			5 => isset($_GET['revision']) ? sprintf(__('%s restored to revision from %s', 'request-a-quote') , $this->sing_label, wp_post_revision_title((int)$_GET['revision'], false)) : false,
			6 => sprintf(__('%s published. <a href="%s">View %s</a>', 'request-a-quote') , $this->sing_label, esc_url(get_permalink($post_ID)) , $this->sing_label) ,
			7 => sprintf(__('%s saved.', 'request-a-quote') , $this->sing_label) ,
			8 => sprintf(__('%s submitted. <a target="_blank" href="%s">Preview %s</a>', 'request-a-quote') , $this->sing_label, esc_url(add_query_arg('preview', 'true', get_permalink($post_ID))) , $this->sing_label) ,
			9 => sprintf(__('%s scheduled for: <strong>%s</strong>. <a target="_blank" href="%s">Preview %s</a>', 'request-a-quote') , $this->sing_label, date_i18n(__('M j, Y @ G:i','request-a-quote') , strtotime($post->post_date)) , esc_url(get_permalink($post_ID)) , $this->sing_label) ,
			10 => sprintf(__('%s draft updated. <a target="_blank" href="%s">Preview %s</a>', 'request-a-quote') , $this->sing_label, esc_url(add_query_arg('preview', 'true', get_permalink($post_ID))) , $this->sing_label) ,
		);
		return $messages;
	}
	/**
...
?>
			...
			jQuery(document).ready(function($){
					$('a.page-title-action').after('<a id="opt-<?php echo str_replace("_", "-", $this->post_type); ?>" class="add-new-h2" href="<?php echo admin_url('edit.php?post_type=' . $this->menu_entity . '&page=operations_' . $this->post_type); ?>" ><?php esc_html_e('CSV Import/Export', 'request-a-quote'); ?></a>');
	...
	protected function set_args_boxes(){
		...
		$attr_list = get_option($this->app . '_attr_list');
			...
			foreach ($attr_list[$this->post_type] as $kattr => $vattr) {
				if (empty($ent_map_list[$this->post_type]['attrs'][$kattr]) || (!empty($ent_map_list[$this->post_type]['attrs'][$kattr]) && $ent_map_list[$this->post_type]['attrs'][$kattr] != 'hide')) {
					...
					if(!empty($this->boxes[$vattr['mid']]['title'])){
						...
						$this->boxes[$vattr['mid']]['fields'][$kattr]['app'] = $this->app;
						//validation
						if ($vattr['required'] == 1) {
		...
		$ext_list = get_option($this->app . '_ext_field_list');
	...
	public function duplicate_entity(){
		if(!empty($_GET['entity_id']) && wp_verify_nonce(sanitize_text_field($_REQUEST['_wpnonce']),'duplicate_'.$_GET['entity_id'])){
						...
						foreach($attr_list[$post->post_type] as $kattr => $myattr){
							if(in_array($kattr,$ent_list[$post->post_type]['unique_keys'])){
								...
								elseif($meta_value == 'concat'){
									$attr_val = emd_get_hidden_func('concat',$this->app,$myattr['concat_string'],$id);
									update_post_meta($id,$kattr,$attr_val);
								}
							}
							else {
	...
	public function duplicate_link($actions, $post){
		if($post->post_type == $this->post_type && in_array($post->post_status, Array('publish','future','private')) && current_user_can('edit_' . $post->post_type . 's')){
			...
			$actions['duplicate'] = '<a href="'. esc_url($duplicate_url) .'" title="'. __('Duplicate','request-a-quote') .'">'.__('Duplicate','request-a-quote').'</a>';
	    	}
		return $actions;
	}
}
