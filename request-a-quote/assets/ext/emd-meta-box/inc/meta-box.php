...
if ( ! class_exists( 'EMD_Meta_Box' ) )
{
	...
	class EMD_Meta_Box
	{
		...
		{
			$jqvalid_msg['required'] = __('This field is required.','request-a-quote');
			$jqvalid_msg['remote'] = __('Please fix this field.','request-a-quote');
			$jqvalid_msg['email'] = __('Please enter a valid email address.','request-a-quote');
			$jqvalid_msg['url'] = __('Please enter a valid URL.','request-a-quote');
			$jqvalid_msg['date'] = __('Please enter a valid date.','request-a-quote');
			$jqvalid_msg['dateISO'] = __('Please enter a valid date ( ISO )','request-a-quote');
			$jqvalid_msg['number'] = __('Please enter a valid number.','request-a-quote');
			$jqvalid_msg['digits']= __('Please enter only digits.','request-a-quote');
			$jqvalid_msg['creditcard']= __('Please enter a valid credit card number.','request-a-quote');
			$jqvalid_msg['equalTo']= __('Please enter the same value again.','request-a-quote');
			$jqvalid_msg['maxlength']= __('Please enter no more than {0} characters.','request-a-quote');
			$jqvalid_msg['minlength']= __('Please enter at least {0} characters.','request-a-quote');
			$jqvalid_msg['rangelength']= __('Please enter a value between {0} and {1} characters long.','request-a-quote');
			$jqvalid_msg['range']= __('Please enter a value between {0} and {1}.','request-a-quote');
			$jqvalid_msg['max']= __('Please enter a value less than or equal to {0}.','request-a-quote');
			$jqvalid_msg['min']= __('Please enter a value greater than or equal to {0}.','request-a-quote');
		...
		function show()
		{
			...
			if(isset( $this->validation ) && $this->validation){
				echo '
					...
					summaryMessage : "' . __( 'Please correct the errors highlighted below and try again.', 'request-a-quote' ) . '",';
			}
		...
		function save_post( $post_id )
		{
			...
			$id = $this->meta_box['id'];
			if ( empty( $_POST["nonce_{$id}"] ) || !wp_verify_nonce( sanitize_text_field($_POST["nonce_{$id}"]), "emd-mb-save-{$id}" ) )
				return;
			...
			foreach ( $this->fields as $field )
			{
				...
				$new  = isset( $_POST[$name] ) ? ($single ? wp_kses_post($_POST[$name]) : array_map('wp_kses_post',$_POST[$name])) : ( $single ? '' : array() );
