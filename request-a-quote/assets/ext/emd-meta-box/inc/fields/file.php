	...
	class EMD_MB_File_Field extends EMD_MB_Field
	{
		...
		static function admin_enqueue_scripts()
		{
			...
			wp_localize_script( 'emd-mb-file', 'emdmbFile', array(
				'maxFileUploadsSingle' => __( 'You may only upload maximum %d file', 'request-a-quote' ),
				'maxFileUploadsPlural' => __( 'You may only upload maximum %d files', 'request-a-quote' ),
				'maxFileSizeError' => __('File Size is greater than allowed limit.','request-a-quote'),
				'FileExtError'	=> __('Invalid file type.','request-a-quote'),
			) );
		}
		...
		static function wp_ajax_delete_file()
		{
			...
			if ( $ok )
				...
				wp_send_json_error( __( 'Error: Cannot delete file', 'request-a-quote' ) );
		}
		...
		{
			$i18n_title = apply_filters( 'emd_mb_file_upload_string', _x( 'Upload Files', 'file upload', 'request-a-quote' ), $field );
			$i18n_more  = apply_filters( 'emd_mb_file_add_string', _x( '+ Add new file', 'file upload', 'request-a-quote' ), $field );
			...
			if(!empty($field['max_file_uploads'])){
				$file_settings .= sprintf(__('Max number of files: %s','request-a-quote'),$field['max_file_uploads']);
			}
			if(!empty($field['max_file_size'])){
				$file_settings .= '<br> ' . sprintf(__('Max file size: %s','request-a-quote'),$field['max_file_size']) . ' KB';
			}
			else {
				...
				$file_settings .= '<br> ' . sprintf(__('Max file size: %s','request-a-quote'),$server_size) . ' KB';
			}
			if(!empty($field['mime_type'])){
				$file_settings .= '<br> ' . sprintf(__('File extensions allowed: %s','request-a-quote'),$field['mime_type']);
			}
			// Show form upload
			$html .= sprintf(
				'<div class="%s">
		...
		{
			$i18n_delete = apply_filters( 'emd_mb_file_delete_string', _x( 'Delete', 'file upload', 'request-a-quote' ) );
			$i18n_edit   = apply_filters( 'emd_mb_file_edit_string', _x( 'Edit', 'file upload', 'request-a-quote' ) );
			$li = '
			<li>
